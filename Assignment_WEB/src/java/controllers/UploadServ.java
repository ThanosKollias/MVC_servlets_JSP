...
public class UploadServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            ...
            Part p = request.getPart("file_to_upload");
            String file_name = p.getSubmittedFileName();
            Connection conn = connection.Bankai.getConnection();
            String sql = "INSERT INTO myFile (myFile_name, myFile_blob) VALUES (?,?)";
            
        try {
            PreparedStatement pst = conn.prepareStatement(sql);
            pst.setString(1, file_name);
            pst.setBinaryStream(2,p.getInputStream());
            pst.executeUpdate();
            pst.close();
            conn.close();
